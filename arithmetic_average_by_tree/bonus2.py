...
for locate_method in ["Sequential", "Binary", "Linear_interpolation"]:
    start = TIME.time()

    if locate_method == "Sequential":
        print("Sequential search method to locate Au and Ad")
    elif locate_method == "Binary":
        print("\nBinary search method to locate Au and Ad")
    elif locate_method == "Linear_interpolation":
        print("\nLinear_interpolation method to locate Au and Ad")
            ...
            for val in node.representative:
                ...
                ### Linear interpolation ###
                    ...
                    if up_node.representative[0] == up_node.representative[M]:
                        ...
                        ku = M * (Au - up_node.representative[0]) / (
                                    up_node.representative[M] - up_node.representative[0])
                        # print(ku)
                        # 可能發生 Au 比最小的 up_val 還小的情況，此時使 Cu 對應到最小的 option value
                        if ku >= M:
                            up_option = up_node.option_value[M]
                        ...
                        #     up_option = up_node.option_value[0]
                            ...
                            wu = (up_node.representative[index_ku - 1] - Au) \
                                 / (up_node.representative[index_ku - 1] - up_node.representative[index_ku])
                            up_option = wu * up_node.option_value[index_ku] + (1 - wu) * up_node.option_value[
                                index_ku - 1]
    ...
    print(f"option value = {node_list_tree[0][0].option_value[0]:.4f}")
    print(f"computational time: {end - start:.4f}")
